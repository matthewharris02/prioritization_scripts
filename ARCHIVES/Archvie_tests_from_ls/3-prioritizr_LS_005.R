...
file <- file(file.path(dir_out, "processed", paste0("logs_5km_005.txt")), open = "wt")
sink(file, append = TRUE); sink(file, append = TRUE, type = "message")
for (i in 1:length(budgets)) {
    ...
    fwrite(s, file.path(dir_out, "output", paste0("solution", budgets[i], ".csv")))
    solutions[[i]] <- s
