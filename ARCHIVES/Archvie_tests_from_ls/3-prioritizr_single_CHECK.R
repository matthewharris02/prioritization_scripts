...
# OPTIONS ====
solver = "cbc"     # Which solver: cbc, lpsymphony
opt_gap = 0.05     # Choose gap for solver
opt_threads = 1    # Choose number of threads (ONLY used for CBC solver)
opt_budget = 0.3   # Choose budget to test
...
dir_out <- file.path(dir_wd, paste0("work_in_progress/", "from_ls"))
...
##%#########################################################################%##
log_file <- file(
    file.path(dir_out, "processed", str_glue("log_test_solution_{RES}km_{opt_gap}g_{opt_threads}t.txt")),
    open = "wt")
print(str_glue("Staring logging to {log_file}"))
...
# Ecoregion targets
ecoregions_data <- read_csv(file.path(dir_out, "global_ecoregions_moll.csv"))
...
print(str_glue("Solving took {end-start} long...!"))
...
fwrite(s, file.path(dir_out, "output",
                    str_glue("test_solution_{RES}km_{opt_gap}g_{opt_threads}t.csv")))
