...
runs <- tribble(
    ~runid,        ~ecor, ~drop,
    "x_carbon",     TRUE, c("ft_carbon"),
    "x_water",      TRUE, c("ft_waterquality"),
    "x_water_carbon", TRUE, c("ft_carbon", "ft_waterquality"),
    "x_ecor", FALSE, c()
)
...
# ALL COMBOS TEST
#a <- stringi::stri_list2matrix(do.call(c, lapply(seq_along(ft_all), combn, x = ft_all, simplify = FALSE)),byrow=TRUE)
...
if (!split) {
    ...
    grid_cell <- open_dataset(file.path(dirs["dir_proc"], "global_cells"),
        ...
        select(id, x, y, "ecoregions", all_of(ft_all)) |>
        mutate(
    ...
    grid_cell <- open_dataset(file.path(dirs["dir_proc"], "global_cells"),
                              partitioning = c("ISONUM")) |>
        select(id, x, y, "ecoregions", all_of(ft_global)) |>
        mutate(
...
for (i in 1:dim(runs)[1]) {
    print(glue("Preparing run #{i} of {dim(runs)[[1]]}"))
    run <- runs[i,]
    runid <- run$runid
    opt_ecoregions <- run$ecor
    drop_feature <- run$drop |>unlist()
    print("Characteristics of run:")
    print(glue("Run-id: {runid}"))
    print(glue("Include ecoregions? {opt_ecoregions}"))
    print(glue("Dropping: {drop_feature}"))
    ...
    # runid <- "exc_carbon"           # Additional ID to distinguish runs
    ...
    # Re-create ft_* for selected only

    ft_national <- selected |>
        filter(split == "national") |>
        select(var) |>
        unlist(use.names = FALSE)
    ft_global <- selected |>
    ...
    print(glue("Finished run #[i} of {dim(runs)[[1]]}"))
    rm(features, rij, feat_master)
}
