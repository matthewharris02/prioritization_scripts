...
opt_threads = 8    # Choose number of threads (ONLY for CBC solver)
...
RES = 10 # Resolution: Relative to 1km/30arcsec
...
for (i in 1:length(budgets)) {
    ...
    # Logging
    file <- file(
        file.path(dir_out, "processed", str_glue("log_{budgets[i]}_{RES}.txt")),
        open = "wt")
    ...
    if (write_each == TRUE) {
        fwrite(s, file.path(dir_out, "output", paste0("solution_", budgets[i], ".csv")))
    } # IF write_each
    sink()
    sink(type = "message")
    close(file)
    print(str_glue("DONE!! Solving took {end-start} long...!"))
} # FOR budget
...
writeRaster(r,
            file.path(dir_out, "output/test_solution.tif"),
            overwrite = T)